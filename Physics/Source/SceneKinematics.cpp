...
void SceneKinematics::Update(double dt)
{
	...
	if(Application::IsKeyPressed('c') || Application::IsKeyPressed('C'))
	{
	...
	if (bVpressed == false && (Application::IsKeyPressed('v') || Application::IsKeyPressed('V')))
	{
		bVpressed = true;
	}
	if(bVpressed == true && !Application::IsKeyPressed('v') && !Application::IsKeyPressed('V'))
	{
		bVpressed = false;
	...
	else if(bLButtonState && !Application::IsMousePressed(0))
	{
		...
		for (auto i = m_goList.begin(); i != m_goList.end(); i++)
		{
			...
			Vector3 MousePosition = Vector3((x / w) * m_worldWidth, ((h - y) / h) * m_worldHeight, 0);
			go->vel = m_ghost->vel = m_ghost->pos - MousePosition;
			go->pos = m_ghost->pos;
	...
	for(std::vector<GameObject *>::iterator it = m_goList.begin(); it != m_goList.end(); ++it)
	{
		...
		if(go->active)
		{
			if (go->type == GameObject::GO_BALL)
			{
				...
				{
					GameObject* go2 = (GameObject*)*it2;
					if (go != go2 && go2->type == GameObject::GO_BALL && go2->active)
					{
						float dis = go->pos.DistanceSquared(go2->pos);
						float rad = go2->scale.x * go2->scale.x;
						if (dis < 10 * rad)
						{
							go->vel += -(go->pos - go2->pos) * 5;
							go2->vel += -(go2->pos - go->pos) * 5;
						}
						if (dis < rad)
						{
							go->vel *= -1;
